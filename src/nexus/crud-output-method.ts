...
import { createQueryBuilder, QueryBuilderConfig } from '../query-builder'
...
function defineFindManyField(
  ...
  const { tableName: entityTableName } = conn.getMetadata(entity)
  const type = config.type || getEntityTypeName(entity)

  const resolver: FindManyResolver<any> = async (_, args, ctx, info) => {
    const joins = grapQLInfoToEntityJoins(info, entity, schemaBuilder)
